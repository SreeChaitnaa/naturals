...
function SaveCustomer() {
    if ($('#New_MobileNo').val() == '') {
        toastr.error("Please enter the Mobile No", "Validation");
        $('#New_MobileNo').focus();
        return false;
    }
    if ($('#New_MobileNo').val().length != 10) {
        toastr.error("The Mobile Number should be 10 digits", "Validation");
        $('#New_MobileNo').focus();
        return false;
    }
    if ($('#New_CustomerName').val() == '') {
        toastr.error("Please enter the customer Name", "Validation");
        $('#New_CustomerName').focus();
        return false;
    } if ($('#Gender').val() == '' || $('#Gender').val() == '0' || $('#Gender').val() == null) {
        toastr.error("Please select the Gender", "Validation");
        $('#Gender').focus();
        return false;
    }
    if ($('#Source').val() == '' || $('#Source').val() == '0' || $('#Source').val() == null) {
        toastr.error("Please select the Source", "Validation");
        $('#Source').focus();
        return false;
    }
    var customer = {
        MobileNo: $('#New_MobileNo').val(),
        CustomerName: $('#New_CustomerName').val(),
        MailId: $('#MailId').val(),
        DOB: $('#DOB').val(),
        DOW: $('#DOW').val(),
        Gender: $('#Gender').val(),
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                cleartextboxDetails();
                ClosecreateWindows();
                toastr.success("Customer details saved...", "Success");
                $('#ISNEWCustID').val("1");
                if (result.CustomerID != null) {
                    var CustomerListTemp = [];
                    $.ajax({
                        ...
                        success: function (data) {
                            CustomerListTemp = data;
                        }
                    })

                    for (i = 0; i < CustomerListTemp.length; i++) {
                        //if (CustomerListTemp[i].label.toUpperCase() == name) {
                        $("#BCustomerName").val(CustomerListTemp[i].label);
                        $("#CustomerID").val(CustomerListTemp[i].value);
                        $("#txtGender").val(CustomerListTemp[i].Gender);
                        $("#MembershipCustomer").val(CustomerListTemp[i].Membership);
                        $("#CustSalonName").val(CustomerListTemp[i].CustSalonName);
                        $("#isMemeberCustomer").val(0);
                        $('#InvoiceInitiateTime').val(CustomerListTemp[i].CurrentTime);
                        $('#CustMobileNo').val(CustomerListTemp[i].MobileNo);
                ...
                toastr.warning("Customer details not saved...", "Fail");
            }

        },
        error: function (xhr, textStatus, error) {
            toastr.error("There is some error occur", "Error");
        }
    });
