...
sections_map = {
  ...
  "Hair Cuts & Styles": ["cut", "blow", "beard", "bangs", "hair styl", "shave", "tongs", "hair do",
...
function calcPayments(bill, gst_percent) {
  ...
  if(adv_amount > 0){
    payment_types.add("NRSPackage");
    nrs_pkg += adv_amount || 0;
    bill_ph = "NotAPhone";
  }
        ...
        if (non_group_reports.includes(reportType)) {
          if (reportType === "services") {
            ...
            row = {
              ...
              Tax: Number((netSalesSum * gst_percent).toFixed(2)),
              Gross: Number((netSalesSum * (1 + gst_percent)).toFixed(2)),
  ...
  if(nrs_only) {
    ...
    if (! is_update) {
      ...
      if(is_ylg()){
        ...
        summary += "  NRS Package Redemption: " + today.NRSPackage + "\n";
      }
